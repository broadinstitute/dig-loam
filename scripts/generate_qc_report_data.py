...
def main(args=None):
	...
	with open(args.out,'w') as f:
		...
		text=[
			...
			r"		\item Freq = Minor allele frequency (MAF) range",
			r"		\item Unpl = Chromosome = 0",
			r"		\item Auto = Autosomal variants",
			r"		\item X = X chromosome non-pseudoautosomal region (non-PAR) variants",
			r"		\item Y = Y chromosome variants",
			r"		\item X(PAR) = X chromosome pseudoautosomal (PAR) region variants",
			r"		\item Mito = Mitochodrial variants",
			r"		\item InDel = Insertion/Deletion variants (I/D or D/I alleles)",
			r"		\item Multi = Multiallelic variants (2 or more alternate alleles)",
			r"		\item Dup = Duplicated variants with respect to position and alleles",
			r"	\end{TableNotes}",
			...
			r"		every head row/.append style={before row={\caption{Summary of raw variants by frequency and classification}\label{table:variantsSummaryTable}\\\insertTableNotes\\\\\toprule}, after row=\midrule\endfirsthead},",
			r"		every first row/.append style={before row={\multicolumn{12}{c}{}\\ \caption[]{... continued from previous page}\\\\\toprule}, after row=\midrule\endhead},",
