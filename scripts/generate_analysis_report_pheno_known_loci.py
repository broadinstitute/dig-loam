			...
			for cohort in top:
				for model in top[cohort]:
					...
					text.extend([
								...
								r"	\end{center}",
								r"	\label{table:topKnownLoci" + args.pheno_name.replace(" ","").replace("_","") + cohort.replace("_","") + model.replace("+","").replace(" ","").replace("_","\_") + r"}",
								r"\end{table}"])
					f.write("\n"); f.write("\n".join(text).encode('utf-8')); f.write("\n")
	
					text = r"\ExecuteMetaData[\currfilebase.input]{"  + args.pheno_long_name.replace(" ","-") + r"-top-known-loci-in-" + cohort.replace("_","-") + r"}"
					...
					text.extend(["%</"  + args.pheno_long_name.replace(" ","-") + r"-top-known-loci-in-" + cohort.replace("_","-") + "-" + model.replace("_","-").replace("+","-") + r">"])
