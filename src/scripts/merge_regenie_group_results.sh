...
(for chr in "${array[@]}"; do \
	...
	zcat $resultsFile | awk -v c=$chr 'BEGIN { OFS="\t" } { $1=c; print $0 }' | sed '1d'; \
done) | awk '{ if($1 == "X") { idx=23 } else { if($1 == "Y") { idx=24 } else { if($1 == "XY") { idx=25 } else { if($1 == "MT") { idx=26 } else { idx=$1 } } } } print idx"\t"$0 }' | sort -T . -n -k1,1 -k3,3 | cut -f2- \
) | bgzip -c > $out
