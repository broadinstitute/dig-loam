	...
	if len(knockout_cohorts) > 0:
		...
		for cohort in knockout_cohorts:
			...
			mt = mt.annotate_entries(
				GT = hl.cond((mt.COHORT == cohort) & (mt['ls_filters']['ls_knockouts_' + cohort]['exclude'] == 1), hl.null(hl.tcall), mt.GT),
				AB = hl.cond((mt.COHORT == cohort) & (mt['ls_filters']['ls_knockouts_' + cohort]['exclude'] == 1), hl.null(hl.tfloat64), mt.AB),
				AB50 = hl.cond((mt.COHORT == cohort) & (mt['ls_filters']['ls_knockouts_' + cohort]['exclude'] == 1), hl.null(hl.tfloat64), mt.AB50),
				GQ = hl.cond((mt.COHORT == cohort) & (mt['ls_filters']['ls_knockouts_' + cohort]['exclude'] == 1), hl.null(hl.tfloat64), mt.GQ),
				GTT = hl.cond((mt.COHORT == cohort) & (mt['ls_filters']['ls_knockouts_' + cohort]['exclude'] == 1), hl.null(hl.tcall), mt.GTT),
				NALTT = hl.cond((mt.COHORT == cohort) & (mt['ls_filters']['ls_knockouts_' + cohort]['exclude'] == 1), hl.null(hl.tint32), mt.NALTT),
				DS = hl.cond((mt.COHORT == cohort) & (mt['ls_filters']['ls_knockouts_' + cohort]['exclude'] == 1), hl.null(hl.tint32), mt.DS)
			)
