object QcReport extends loamstream.LoamFile {
  ...
  def QcReport(): Unit = {
    ...
    //      .in(arrayStores.map(e => e._2).map(e => e.rawData.imissRemove.get).toSeq :+ qcReportStores.figureData.samplesUpsetPlotPdf.get :+ qcReportStores.tablesData.rawVariantsSummary :+ qcReportStores.figureData.variantsUpsetPlotPdf.get)
    //      .out(qcReportStores.texData.data)
    ...
    //    --bim ${bimStrings(0)}
    //    --out ${qcReportStores.texData.data}"""
    //    .in(arrayStores.map(e => e._2).map(e => e.rawData.imissRemove.get).toSeq ++ arrayStores.map(e => e._2).flatMap(e => e.refData.plink.get.data.local.get).toSeq :+ qcReportStores.tablesData.rawVariantsSummary)
    //    .out(qcReportStores.texData.data)
    //    .tag(s"${qcReportStores.texData.data}".split("/").last)
    //
    //  }
