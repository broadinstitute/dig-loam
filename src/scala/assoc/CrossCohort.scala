object CrossCohort extends loamstream.LoamFile {
  ...
  def CrossCohortPrep(configMeta: ConfigMeta, configCohort: ConfigCohort): Unit = {
    ...
    drmWith(imageName = s"${utils.image.imgR}") {
      ...
      .in(arrayStores(arrayCfg).filteredPlink.data :+ arrayStores(arrayCfg).phenoFile.local.get :+ arrayStores(arrayCfg).ancestryMap)
      .out(metaKinshipStores(configMeta).metaCohort(configCohort).samples)
    ...
    drmWith(imageName = s"${utils.image.imgTools}", cores = projectConfig.resources.standardPlink.cpus, mem = projectConfig.resources.standardPlink.mem, maxRunTime = projectConfig.resources.standardPlink.maxRunTime) {
      ...
      cmd"""${utils.binary.binPlink} --bfile ${arrayStores(arrayCfg).filteredPlink.base} --allow-no-sex --extract ${metaKinshipStores(configMeta).commonVariants} --keep ${metaKinshipStores(configMeta).metaCohort(configCohort).samples} --make-bed --out ${metaKinshipStores(configMeta).metaCohort(configCohort).base} --memory ${projectConfig.resources.standardPlink.mem * 0.9 * 1000} --seed 1"""
      .in(arrayStores(arrayCfg).filteredPlink.data :+ metaKinshipStores(configMeta).metaCohort(configCohort).samples :+ metaKinshipStores(configMeta).commonVariants)
