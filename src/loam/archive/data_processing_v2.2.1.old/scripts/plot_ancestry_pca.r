...
color[grep(args$id,levels(data$GROUP))]<-"#000000"
pdf(args$out,width=7,height=7)
p<-ggplot() +
	geom_point(data=data[which(data$GROUP != args$id),],aes(PC1,PC2,color=GROUP), size=2) +
	geom_point(data=data[which(data$GROUP == args$id),],aes(PC1,PC2,color=GROUP), size=2) +
...
p<-ggplot() +
	geom_point(data=data[which(data$GROUP != args$id),],aes(PC2,PC3,color=GROUP), size=2) +
	geom_point(data=data[which(data$GROUP == args$id),],aes(PC2,PC3,color=GROUP), size=2) +
	scale_colour_manual(name = "GROUP",values = color) +
	theme_bw() +
	guides(col = guide_legend(override.aes = list(shape = 15, size = 10))) +
