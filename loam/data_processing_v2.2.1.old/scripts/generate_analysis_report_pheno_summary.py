...
def main(args=None):
	...
	for cohort in model_files.keys():
		...
		for model in model_files[cohort].keys():
			...
			row.extend([df_temp[df_temp[args.sex_col].isin([args.male_code,"M","m","Male","male"])].shape[0]])
			row.extend([df_temp[df_temp[args.sex_col].isin([args.female_code,"F","f","Female","female"])].shape[0]])
			if dichotomous:
...
if __name__ == "__main__":
	...
	requiredArgs.add_argument('--male-code', help='a code for males', required=True)
	requiredArgs.add_argument('--female-code', help='a code for females', required=True)
